...
➜ Smooth Booking Process with **on-page booking** option
➜ Customized Travel and Tour Package creation including **itinerary builder**
➜ Compatibility with some of the **best travel agency WordPress themes**
➜ **SEO Friendly & compatibility** with popular plugins like Yoast SEO and Rank Math
➜ Hassle-free payments through **secure and popular payment gateways**
➜ Expansion on your website's functionalities with **additional features and extensions**
...
* **[💁‍♂️ Support Forum](https://wptravel.io/contact/)**: If you have specific questions that aren't covered by our tutorials or documentation, feel free to reach out to our support team. We're here to assist you with any issues or queries you may have. Additionally, chat support is available during office hours for more immediate assistance.

* **[🗣️ Active Facebook Community](https://www.facebook.com/groups/wptravelcommunity/)**: Join our Facebook community to connect with other WP Travel Plugin users. Engage in discussions about travel website development, marketing, and more. It's a great place to share experiences and learn from others.
...
* **[📄 Documentation](https://wptravel.io/wp-travel-documentations/)**: We provide detailed written documentation that outlines the usage of WP Travel. This documentation includes explanations, instructions, and examples to assist you in understanding and utilizing the plugin effectively.
...
Fixes:
* Fix: Resolved an issue with the checkout DOB (Date of Birth) date picker that prevented users from selecting past dates.

= 12.0.2 =
Release Date: 4th September 2026

Fixes:
* Fix: Unauthenticated arbitrary booking payment reset.
* Fix: Authenticated (Subscriber+) booking payment-state tampering via IDOR.
* Fix: Issue caused by is_plugin_active() when the plugin is network activated on multisite.
* Fix: Issue caused by get_plugin_data() returning null, by adding a file_exists() check.

Special thanks to Revanth Hari Narayana Matte for reporting the issue on the plugin.

= 12.0.1 =
Release Date: 19th August 2026

Fixes
* Fixed a checkout issue affecting inventory-enabled trips.
...
* Fixed the responsive layout issue affecting date listings in the trip template.
* Resolved a deprecation warning on the Booking Report page.

Enhancements:
* Improved sanitization logic in the pricing update function to prevent unintended data loss.
* Added the wp_travel_show_min_price_from_selected_pricing_cat_single_trip_template filter to allow displaying the minimum price from a selected pricing category on the single trip template.
* Added the wp_travel_set_pricing_cat_to_show_min_price_single_trip_template filter to define which pricing category should be used for minimum price display on the single trip template.

**For more details, please refer to our [release note](https://wptravel.io/wp-travel-plugin-v11-9-0-update-release-note).**
...
* Security: Improved payment request validation and authorization checks.
* Security: Enhanced booking access controls.
...
Fixes
* Added an authentication + ownership check to "wp_travel_cancel_booking_callback()".
* Removed "wp_ajax_nopriv_wp_travel_cancel_booking" registration.
...
Fixes:
* Updated the Archive Page Search Form so that the Location taxonomy is displayed in a parent–child hierarchical structure within select options.
* Audited AJAX requests and added appropriate security validations to help protect against potential attacks.

Enhancements:
* Added a condition to make the dashboard widget visible only to site admins.
* Added the wp_travel_load_text_domain_from_mo_files filter to control loading translations from .mo files.

**For more details, please refer to our [release note](https://wptravel.io/wp-travel-plugin-v11-6-0-update-release-note).**
