...
add_action( 'init', function () {
    ...
    $plugin_file = 'wp-travel-pro/wp-travel-pro.php';
    ...
    if ( version_compare( $plugin_data['Version'], '11.0.0', '>=' ) ) {
        ...
        add_shortcode( 'wp_trip_calendar_view', 'wptravel_custom_calendar_shortcode_legacy' );
    }
...
function wptravel_custom_calendar_shortcode($atts) {
    ...
    global $wpdb;
        ...
        */
        $calendar_initial_view = apply_filters(
            'wp_travel_trip_calendar_initial_view',
            'dayGridMonth'
        );
        ...
        * Calendar toolbar
        */
        $calendar_header_toolbar = apply_filters(
            'wp_travel_trip_calendar_header_toolbar',
            array(
        ...
        */
        $calendar_day_names = apply_filters(
            'wp_travel_trip_calendar_day_names',
            array(
                0 => 'Sun',
                1 => 'Mon',
                2 => 'Tue',
        ...
        */
        $calendar_first_day_js = wp_json_encode(
            (int) $calendar_first_day
        );

        $calendar_initial_view_js = wp_json_encode(
            $calendar_initial_view
        );

        $calendar_header_toolbar_js = wp_json_encode(
            $calendar_header_toolbar
        );

        $calendar_day_names_js = wp_json_encode(
            $calendar_day_names
        );
        ...
        wp_add_inline_script(
            ...
            'document.addEventListener("DOMContentLoaded",function(){' .
                ...
                * Calendar settings
                */
                'const calendarFirstDay=' . $calendar_first_day_js . ';' .
                'const calendarInitialView=' . $calendar_initial_view_js . ';' .
                'const calendarHeaderToolbar=' . $calendar_header_toolbar_js . ';' .
                'const calendarDayNames=' . $calendar_day_names_js . ';' .

                /*
                * Calendar element
                */
                'const el=document.getElementById("wp-trip-calendar");' .
                'if(!el) return;' .
                ...
                'const cal=new FullCalendar.Calendar(el,{' .
                    ...
                    */
                    'dayHeaderContent:function(arg){' .
                        'return calendarDayNames[arg.date.getDay()];' .
                    '},' .
                    ...
                    */
                    'eventClick:function(info){' .
                        ...
                        'const m=document.getElementById("wp-trip-modal");' .
                        'if(!m) return;' .
                        ...
                        * Title
                        */
                        'var t=m.querySelector(".trip-title");' .
                        'if(t) t.textContent=info.event.title;' .


                        /*
                        * Pax
                        */
                        'var px=m.querySelector(".trip-pax");' .
                        'if(px) px.textContent=trip.pax||"";' .


                        /*
                        * Price
                        */
                        'var pr=m.querySelector(".trip-price");' .
                        'if(pr) pr.innerHTML=trip.price||"";' .


                        /*
                        * Trip URL
                        */
                        'var a=m.querySelector(".trip-link");' .
                        'if(a) a.href=trip.tripUrl||"#";' .
                        ...
                        */
                        ...
                        'var ratingEl=m.querySelector(".trip-rating");' .
...
$calendar_day_names = apply_filters(
    ...
    // Automatically rotate weekday names
    $calendar_day_names = array_merge(
        array_slice($calendar_day_names, $calendar_first_day),
        array_slice($calendar_day_names, 0, $calendar_first_day)
    );

    // Month names
    $calendar_months = apply_filters(
        'wp_travel_calendar_month_names',
        array(
            'January',
            'February',
            'March',
    ...
    const CALENDAR_DAY_NAMES = <?php
    echo wp_json_encode($calendar_day_names);
    ?>;

    const CALENDAR_FIRST_DAY = <?php echo (int) $calendar_first_day; ?>;

    const CALENDAR_MONTHS = <?php
    echo wp_json_encode($calendar_months);
    ?>;
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        function renderMonths() {
            ...
            CALENDAR_MONTHS.forEach((month, index) => {
                ...
                html += `
                    <li
                        data-month="${index}"
                        class="${index === current.getMonth() ? 'active' : ''}"
                    >
                        ${month}
                    </li>
                `;
        ...
        */
        function renderCalendar() {
            ...
            * Saturday  = 6
            *
            * Convert it based on our configured
            ...
            */
            ...
            const firstDay =
                (firstDayOfMonth - CALENDAR_FIRST_DAY + 7) % 7;
        ...
        document.addEventListener("click", function(e) {
            ...
            */
            if (e.target.tagName === "LI") {
                ...
                document
                    .querySelectorAll(".cal-sidebar li")
                    .forEach(li => {
...
function wptravel_custom_calendar_shortcode_legacy($atts) {
